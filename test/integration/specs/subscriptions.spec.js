...
describe('when using the subscriptions resource', () => {
    ...
    it('I can create a subscription with ID', async() => {
        ...
        const {id: id, ...data} = createSubscriptionData(true, sharedData);
        const subscription = await apiInstance.subscriptions.create({id: id, data: data});
        testIds.with = id;
    ...
    it('I can create a subscription without ID', async() => {
        const sharedData = await prepareStub();
        const {id: id, ...data} = createSubscriptionData(false, sharedData);
        const subscription = await apiInstance.subscriptions.create({id: id, data: data});
    ...
    it('I can update a subscription by its ID', async() => {
        const sharedData = await prepareStub();
        const {id: id, ...data} = createSubscriptionData(false, sharedData);
        const subscription = await apiInstance.subscriptions.update({id: testIds.with, data: data});
        expect(subscription.response.status).to.be.equal(200);
    });
    ...
    it('I can switch a subscription by its ID', async() => {
        ...
        const subscriptionSwitch = await apiInstance.subscriptions.switch({id: testIds.with, data: data});
    ...
    it('I can create a lead source for subscription by its ID', async() => {
        ...
        const leadSource = await apiInstance.subscriptions.createLeadSource({id: testIds.with, data: data});
